...
#define APRIL_CUSTOM_WINDOW_CLASS L"AprilWin32Window"
...
#define CUSTOM_WINDOW ((CustomWindow*)april::window)
...
namespace april
{
	...
	void CustomWindow::_systemCreate(int width, int height, bool fullscreen, chstr title, Window::Options options)
	{
		...
		windowClass.lpfnWndProc = &CustomWindow::_mainProcessCallback;
		windowClass.hInstance = hinst;
		windowClass.hCursor = this->defaultCursor;
		windowClass.lpszClassName = APRIL_CUSTOM_WINDOW_CLASS;
		...
		// create window
		this->hWnd = CreateWindowExW(exstyle, APRIL_CUSTOM_WINDOW_CLASS, this->title.wStr().c_str(), style, x, y, width, height, NULL, NULL, hinst, NULL);
	...
	void CustomWindow::_systemDestroy()
	{
		...
		if (this->hWnd != 0)
		{
			...
			UnregisterClassW(APRIL_CUSTOM_WINDOW_CLASS, GetModuleHandle(0));
			this->hWnd = 0;
		}
	}

	void CustomWindow::setTitle(chstr title)
	{
	...
	HCURSOR CustomWindow::getCursorHandle() const
	{
	...
	void CustomWindow::_systemSetResolution(int width, int height, bool fullscreen)
	{
	...
	void CustomWindow::checkEvents()
	{
		...
		{
			SetCursor(this->isCursorVisible() ? this->defaultCursor : NULL);
			this->refreshCursorRequested = false;
		}
		MSG message;
...
#ifdef _CUSTOM_XINPUT
	void CustomWindow::_checkXInputControllerStates()
	{
	...
	void CustomWindow::_adjustWindowSizeForClient(int x, int y, int& width, int& height, DWORD style, DWORD exstyle)
	{
		RECT rect;
		rect.left = x;
		rect.top = y;
	...
	void CustomWindow::queueControllerInput(const ControllerEvent::Type& type, int controllerIndex, const Button& buttonCode, float axisValue)
	{
	...
	LRESULT CALLBACK CustomWindow::_mainProcessCallback(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_SIZE:
			...
			{
				RECT clientRect;
				GetClientRect(CUSTOM_WINDOW->hWnd, &clientRect);
				CUSTOM_WINDOW->width = clientRect.right - clientRect.left;
				CUSTOM_WINDOW->height = clientRect.bottom - clientRect.top;
				CUSTOM_WINDOW->_setRenderSystemResolution(april::window->getWidth(), april::window->getHeight(), april::window->isFullscreen());
				ShowWindow(CUSTOM_WINDOW->hWnd, SW_SHOW);
				UpdateWindow(CUSTOM_WINDOW->hWnd);
	...
	LRESULT CALLBACK CustomWindow::childProcessCallback(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		if (!april::window->isCreated()) // don't run callback processing if window was "destroyed"
		{
			return 1;
		...
		case WM_TOUCH: // (Win7+ only)
			if (wParam > 0 && _getTouchInputInfo != NULL && _closeTouchInputHandle != NULL)
			{
				...
				CUSTOM_WINDOW->_mouseMessages = 5;
				if (_getTouchInputInfo((HTOUCHINPUT)lParam, wParam, touches, sizeof(TOUCHINPUT)))
				{
					...
					CUSTOM_WINDOW->cursorPosition.set((float)_systemCursorPosition.x, (float)_systemCursorPosition.y);
		...
		case WM_KEYDOWN:
			if (_altKeyDown)
			{
				...
				if (wParam == VK_RETURN)
				{
					CUSTOM_WINDOW->_systemToggleHotkeyFullscreen(); // this comes from the main thread so it can directly call the needed function
					return 0;
				}
			}
		...
		case WM_RBUTTONDOWN:
			...
			{
				CUSTOM_WINDOW->_mouseMessages = 0;
				if (!april::window->isFullscreen())
				{
					SetCapture((HWND)april::window->getBackendId());
				}
				...
				{
					CUSTOM_WINDOW->_updateCursorPosition();
		...
		case WM_MOUSEMOVE:
			if (!_touchEnabled || (GetMessageExtraInfo() & MOUSEEVENTF_FROMTOUCH) != MOUSEEVENTF_FROMTOUCH)
			{
				if (CUSTOM_WINDOW->_mouseMessages > 0)
				{
					--CUSTOM_WINDOW->_mouseMessages;
				}
				if (CUSTOM_WINDOW->_mouseMessages == 0)
				{
					april::window->queueInputModeChange(InputMode::Mouse);
		...
		case WM_SETCURSOR:
			...
			{
				HCURSOR cursor = CUSTOM_WINDOW->getCursorHandle();
				if (cursor != GetCursor())
				{
					SetCursor(cursor);
				}
