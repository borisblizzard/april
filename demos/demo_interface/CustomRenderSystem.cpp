...
#define APRIL_DX9_CHILD L"AprilCustomChild"

#define IS_WINDOW_RESIZABLE ((april::window->getName() == april::WindowType::Win32.getName() || april::window->getName() == "Custom") && april::window->getOptions().resizable)
...
namespace april
{
	...
	D3DPRIMITIVETYPE CustomRenderSystem::_dx9RenderOperations[] =
	{
		D3DPT_TRIANGLELIST,
		D3DPT_TRIANGLESTRIP,
		D3DPT_LINELIST,
	...
	void CustomRenderSystem::_deviceAssignWindow(Window* window)
	{
		...
		if (resizable)
		{
			...
			wc.lpfnWndProc = &CustomWindow::childProcessCallback;
		...
		{
			this->d3dDevice->SetRenderTarget(0, ((CustomTexture*)this->_currentIntermediateRenderTexture)->_getSurface());
		}
		this->d3dDevice->BeginScene();
	}

	void CustomRenderSystem::_deviceReset()
	{
		...
		while (april::application->getState() == Application::State::Running)
		{
			...
			{
				hlog::errorf(logTag, "Failed to reset device! CustomRenderSystem::reset() hresult: 0x%08X", hr);
			}
		}
		this->_deviceSetup();
	...
	void CustomRenderSystem::_tryAssignChildWindow()
	{
	...
	void CustomRenderSystem::_tryUnassignChildWindow()
	{
	...
	void CustomRenderSystem::_deviceSetupDisplayModes()
	{
	...
	int CustomRenderSystem::getVRam() const
	{
		if (this->d3dDevice == NULL)
		{
			return 0;
	...
	Texture* CustomRenderSystem::_deviceCreateTexture(bool fromResource)
	{
		return new CustomTexture(fromResource);
	}

	void CustomRenderSystem::_deviceChangeResolution(int width, int height, bool fullscreen)
	{
	...
	void CustomRenderSystem::_setDeviceViewport(cgrecti rect)
	{
	...
	void CustomRenderSystem::_setDeviceTexture(Texture* texture)
	{
		if (texture != NULL)
		{
			CustomTexture* currentTexture = (CustomTexture*)texture;
	...
	void CustomRenderSystem::_setDeviceTextureFilter(const Texture::Filter& textureFilter)
	{
	...
	void CustomRenderSystem::_setDeviceTextureAddressMode(const Texture::AddressMode& textureAddressMode)
	{
	...
	void CustomRenderSystem::_setDeviceBlendMode(const BlendMode& blendMode)
	{
	...
	void CustomRenderSystem::_setDeviceColorMode(const ColorMode& colorMode, float factor, bool useTexture, bool useColor, const Color& systemColor)
	{
	...
	void CustomRenderSystem::_setDeviceRenderTarget(Texture* texture)
	{
		...
		{
			this->d3dDevice->SetRenderTarget(0, ((CustomTexture*)texture)->_getSurface());
		}
		if (this->deviceState->renderTarget != NULL)
		{
			this->d3dDevice->BeginScene();
	...
	void CustomRenderSystem::_deviceClear(const Color& color, bool depth)
	{
		DWORD flags = D3DCLEAR_TARGET;
		if (depth)
		{
	...
	void CustomRenderSystem::_deviceRender(const RenderOperation& renderOperation, const PlainVertex* vertices, int count)
	{
	...
	void CustomRenderSystem::_deviceRender(const RenderOperation& renderOperation, const TexturedVertex* vertices, int count)
	{
	...
	void CustomRenderSystem::_deviceRender(const RenderOperation& renderOperation, const ColoredVertex* vertices, int count)
	{
	...
	void CustomRenderSystem::_deviceRender(const RenderOperation& renderOperation, const ColoredTexturedVertex* vertices, int count)
	{
	...
	Image::Format CustomRenderSystem::getNativeTextureFormat(Image::Format format) const
	{
	...
	void CustomRenderSystem::_devicePresentFrame(bool systemEnabled)
	{
		if (this->_currentIntermediateRenderTexture != NULL)
		{
			this->d3dDevice->SetRenderTarget(0, this->backBuffer);
		...
		if (hr == D3DERR_DEVICELOST)
		{
			...
			while (april::application->getState() == Application::State::Running)
			{
				...
				if (hr == D3DERR_DEVICENOTRESET)
				{
					...
					{
						hlog::errorf(logTag, "Failed to reset device!, context: CustomRenderSystem::_devicePresentFrame() hresult: 0x%08X", hr);
					}
				}
	...
	void CustomRenderSystem::_deviceCopyRenderTargetData(Texture* source, Texture* destination)
	{
		...
		CustomTexture* dx9Source = (CustomTexture*)source;
		CustomTexture* dx9Destination = (CustomTexture*)destination;
	...
	void CustomRenderSystem::_deviceTakeScreenshot(Image::Format format, bool backBufferOnly)
	{
		IDirect3DSurface9* surface = this->backBuffer;
		if (this->_currentIntermediateRenderTexture != NULL)
		{
			surface = ((CustomTexture*)this->_currentIntermediateRenderTexture)->_getSurface();
		}
		if (!backBufferOnly && this->deviceState->renderTarget != NULL)
		{
			surface = ((CustomTexture*)this->deviceState->renderTarget)->_getSurface();
		}
